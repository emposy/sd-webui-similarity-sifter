...
class SimilaritySifter(scripts.Script):
    ...
    def save_image(self, p: StableDiffusionProcessingTxt2Img, image, job_timestamp):
        ...
        job_datetime = datetime.strftime(datetime.strptime(job_timestamp, "%Y%m%d%H%M%S"), "%Y-%m-%d_%H-%M-%S")
        output_folder = os.path.join(shared.opts.data.get("ss_output_dir", ss_output_dir), f"{job_datetime}_{p.seed}")
    ...
    def postprocess_image(self, p: StableDiffusionProcessingTxt2Img, pp: scripts.PostprocessImageArgs, is_enabled, uploaded_image, remove_low_similarity, similarity_threshold):
        if not is_enabled or uploaded_image is None:
            return
        similarity = self.calculate_similarity(uploaded_image, pp.image)
        pp.image.info["parameters"] = Processed(p, []).infotext(p, p.iteration) # 画像情報を先に保存しておく（あとで上書きされる）
        pp.image.info["SS_similarity"] = similarity
        p.extra_generation_params['SS similarity'] = similarity

        if p.n_iter > 1 and (not remove_low_similarity or similarity >= similarity_threshold):
            self.save_image(p, pp.image, shared.state.job_timestamp)
    
    def postprocess(self, p, processed: Processed, is_enabled, uploaded_image, remove_low_similarity, similarity_threshold):
        if not is_enabled or uploaded_image is None:
            return
